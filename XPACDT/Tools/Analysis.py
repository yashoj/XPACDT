...
def output_data(header, file_output, form, times, bins, results, two_d=False):
    ...
    gnuplotfile = open(file_output + ".plt", "w")
...
#                                      str(data[1+i*len(bins[1])+j]) + " \n")
        ...
        # Regular output, just one line per timestep
            ...
            # TODO: Adaptive number of columns to plot!
            gnuplotfile.write("set xlabel 'time / au'\n")
            gnuplotfile.write("set ylabel 'TODO'\n")
            gnuplotfile.write("set output '" + file_output.split("/")[-1] + ".eps'\n")
            gnuplotfile.write("plot '" + file_output.split("/")[-1] + "' ")
            gnuplotfile.write("using 1:2 w l ls 1 title 'First Value', '' using 1:2:3 w yerrorbars ls 1 title ''")
