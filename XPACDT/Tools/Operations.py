...
class Operations(object):
    ...
    def __init__(self, operation_string, print_help=False):
        ...
        for i, op_string in enumerate(operation_string.split('+')[1:]):
            ...
            if ops[0] == 'pos' or ops[0] == 'position':
                ...
                self.__operation_list['momentum' + key_index] = self._arguments_momentum(ops[1:])

            elif ops[0] == 'vel' or ops[0] == 'velocity':
                self.__operation_list['momentum' + key_index] = self._arguments_position(ops[1:] + ['-v'])

            elif ops[0] == 'state':
                self.__operation_list['electronic_state' + key_index] = self._arguments_electronic_state(ops[1:])

            elif ops[0] == 'energy':
                self.__operation_list['energy' + key_index] = self._arguments_energy(ops[1:])
            else:
    ...
    def apply_operation(self, log_nuclei):
        ...
        for op, options in self.operation_list.items():
            ...
            value *= getattr(self, operation)(options, log_nuclei)

        return np.array(value).flatten()

    def _arguments_position(self, arguments):
        """
    ...
    def _position(self, opts, log_nuclei):
        ...
        if opts.proj is not None:
            current_value = self._projection(opts.proj, current_value)

        return np.array(current_value).flatten()


    def _arguments_momentum(self, arguments):
        """
    ...
    def _momentum(self, opts, log_nuclei):
    ...
    def _projection(self, options, values):
    ...
    def _arguments_electronic_state(self, arguments):
        """
    ...
    def _electronic_state(self, opts, log_nuclei):
    ...
    def _arguments_energy(self, arguments):
        """
